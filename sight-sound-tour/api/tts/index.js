// Azure Functions (Node 22, CommonJS) — Speech TTS via MSI (App Service endpoint first, IMDS fallback)

async function getBearerToken(context, region) {
  // Speech uses region-specific STS, but AAD resource is still cognitiveservices
...
module.exports = async function (context, req) {
  try {
    ...
    const voice = (req.body?.voice || req.query?.voice || "").toString() || "en-US-JennyNeural";
    ...
    const token = await getBearerToken(context, region);

    // Speech TTS endpoint (Neural)
    ...
    // Pass through status; return audio on success
    context.res = {
      status: resp.status,
      headers: {
